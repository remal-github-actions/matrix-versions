...
var external_tty_ = __webpack_require__(52018);
...
var external_v8_ = __webpack_require__(11493);
...
var __require = /* @__PURE__ */ ((x) => typeof prettier_require !== "undefined" ? prettier_require : typeof Proxy !== "undefined" ? new Proxy(x, {
  ...
  if (typeof prettier_require !== "undefined") return prettier_require.apply(this, arguments);
...
async function importModuleDefault(file) {
  const module = await __webpack_require__(69512)((0,external_url_.pathToFileURL)(file).href);
...
function getMessage(key2, parameters, self) {
  ...
  return Reflect.apply(external_util_.format, null, parameters);
}
...
function getPackageScopeConfig(resolved) {
  ...
  while (true) {
    ...
    const packageConfig = read2((0,external_url_.fileURLToPath)(packageJSONUrl), {
      specifier: resolved
    });
...
function emitLegacyIndexDeprecation(url2, packageJsonUrl, base, main) {
  ...
  const packagePath = (0,external_url_.fileURLToPath)(new external_url_.URL(".", packageJsonUrl));
...
Default "index" lookups for the main are deprecated for ES modules.`,
  ...
  } else if (external_path_.resolve(packagePath, main) !== urlPath) {
    external_process_.emitWarning(
...
function moduleResolve(specifier, base, conditions, preserveSymlinks) {
  ...
  if (shouldBeTreatedAsRelativeOrAbsolutePath(specifier)) {
    ...
    try {
      ...
      if (isRemote && !external_module_.builtinModules.includes(specifier)) {
        const error = new ERR_UNSUPPORTED_RESOLVE_REQUEST(specifier, base);
        error.cause = error_;
        throw error;
      }
...
function importFromFile(specifier, parent) {
  ...
  return __webpack_require__(69512)(url2);
}
...
function prettier_stripTrailingHardlineFromDoc(doc2) {
  switch (prettier_get_doc_type_default(doc2)) {
    ...
    case prettier_DOC_TYPE_FILL:
      return { ...doc2, parts: prettier_stripTrailingHardlineFromParts(doc2.parts) };
    case prettier_DOC_TYPE_ARRAY:
...
function inheritLabel(doc2, fn) {
  return doc2.type === prettier_DOC_TYPE_LABEL ? { ...doc2, contents: fn(doc2.contents) } : fn(doc2);
}
...
// src/utils/get-string-width.js
var prettier_notAsciiRegex = /[^\x20-\x7F]/u;
...
var prettier_DOC_FILL_PRINTED_LENGTH = Symbol("DOC_FILL_PRINTED_LENGTH");
function prettier_rootIndent() {
  return { value: "", length: 0, queue: [] };
}
function prettier_makeIndent(ind, options8) {
  return prettier_generateInd(ind, { type: "indent" }, options8);
}
function prettier_makeAlign(indent2, widthOrDoc, options8) {
  ...
  if (widthOrDoc < 0) {
    return prettier_generateInd(indent2, { type: "dedent" }, options8);
  }
  if (!widthOrDoc) {
    return indent2;
  }
  ...
  const alignType = typeof widthOrDoc === "string" ? "stringAlign" : "numberAlign";
  return prettier_generateInd(indent2, { type: alignType, n: widthOrDoc }, options8);
}
...
function prettier_fits(next, restCommands, width, hasLineSuffix, groupModeMap, mustBeFlat) {
  ...
  while (width >= 0) {
    ...
    const docType = prettier_get_doc_type_default(doc2);
    switch (docType) {
      case prettier_DOC_TYPE_STRING:
        out.push(doc2);
      ...
      case prettier_DOC_TYPE_ARRAY:
      case prettier_DOC_TYPE_FILL: {
        const parts = docType === prettier_DOC_TYPE_ARRAY ? doc2 : doc2.parts;
        const end = doc2[prettier_DOC_FILL_PRINTED_LENGTH] ?? 0;
        for (let i = parts.length - 1; i >= end; i--) {
          cmds.push({ mode, doc: parts[i] });
        }
        break;
...
function prettier_printDocToString(doc2, options8) {
  ...
  let pos2 = 0;
  const cmds = [{ ind: prettier_rootIndent(), mode: prettier_MODE_BREAK, doc: doc2 }];
  const out = [];
  let shouldRemeasure = false;
  const lineSuffix2 = [];
  let printedCursorCount = 0;
  ...
  while (cmds.length > 0) {
    ...
    switch (prettier_get_doc_type_default(doc3)) {
      ...
      case prettier_DOC_TYPE_CURSOR:
        ...
        break;
      case prettier_DOC_TYPE_INDENT:
        cmds.push({ ind: prettier_makeIndent(ind, options8), mode, doc: doc3.contents });
        break;
      case prettier_DOC_TYPE_ALIGN:
        cmds.push({
      ...
      case prettier_DOC_TYPE_GROUP:
        switch (mode) {
          ...
          case prettier_MODE_BREAK: {
            shouldRemeasure = false;
            const next = { ind, mode: prettier_MODE_FLAT, doc: doc3.contents };
            ...
            if (!doc3.break && prettier_fits(next, cmds, rem, hasLineSuffix, groupModeMap)) {
              ...
              if (doc3.expandedStates) {
                ...
                if (doc3.break) {
                  cmds.push({ ind, mode: prettier_MODE_BREAK, doc: mostExpanded });
                  break;
                } else {
                  for (let i = 1; i < doc3.expandedStates.length + 1; i++) {
                    if (i >= doc3.expandedStates.length) {
                      ...
                      const cmd = { ind, mode: prettier_MODE_FLAT, doc: state };
                ...
                cmds.push({ ind, mode: prettier_MODE_BREAK, doc: doc3.contents });
              }
            }
            break;
          }
      ...
      case prettier_DOC_TYPE_FILL: {
        const rem = width - pos2;
        const offset = doc3[prettier_DOC_FILL_PRINTED_LENGTH] ?? 0;
        const { parts } = doc3;
        const length = parts.length - offset;
        if (length === 0) {
          break;
        ...
        const content = parts[offset + 0];
        const whitespace = parts[offset + 1];
        const contentFlatCmd = { ind, mode: prettier_MODE_FLAT, doc: content };
        const contentBreakCmd = { ind, mode: prettier_MODE_BREAK, doc: content };
        const contentFits = prettier_fits(
          contentFlatCmd,
          [],
          rem,
          lineSuffix2.length > 0,
        ...
        const whitespaceFlatCmd = { ind, mode: prettier_MODE_FLAT, doc: whitespace };
        const whitespaceBreakCmd = { ind, mode: prettier_MODE_BREAK, doc: whitespace };
        if (length === 2) {
          if (contentFits) {
            cmds.push(whitespaceFlatCmd, contentFlatCmd);
          } else {
        ...
        const remainingCmd = {
          ind,
          mode,
          doc: { ...doc3, [prettier_DOC_FILL_PRINTED_LENGTH]: offset + 2 }
        };
        const firstAndSecondContentFlatCmd = {
          ind,
        ...
        const firstAndSecondContentFits = prettier_fits(
          firstAndSecondContentFlatCmd,
          [],
          rem,
          lineSuffix2.length > 0,
      ...
      case prettier_DOC_TYPE_LINE_SUFFIX_BOUNDARY:
        if (lineSuffix2.length > 0) {
          cmds.push({ ind, mode, doc: prettier_hardlineWithoutBreakParent });
        }
        break;
      case prettier_DOC_TYPE_LINE:
        switch (mode) {
  ...
  if (cursorPlaceholderIndex !== -1) {
    const otherCursorPlaceholderIndex = out.indexOf(
      prettier_CURSOR_PLACEHOLDER,
      cursorPlaceholderIndex + 1
    );
    if (otherCursorPlaceholderIndex === -1) {
      return {
        formatted: out.filter((char) => char !== prettier_CURSOR_PLACEHOLDER).join("")
      };
    }
    const beforeCursor = out.slice(0, cursorPlaceholderIndex).join("");
    const aroundCursor = out.slice(cursorPlaceholderIndex + 1, otherCursorPlaceholderIndex).join("");
...
function optionInfoToSchema(optionInfo, { isCLI, optionInfos, FlagSchema }) {
  ...
  if (isCLI && !optionInfo.array) {
    ...
    parameters.preprocess = (value, schema2, utils) => schema2.preprocess(
      originalPreprocess(Array.isArray(value) ? prettier_at_default(
        /* isOptionalObject */
        false,
        value,
        -1
...
async function printAstToDoc(ast, options8) {
  ...
  if (options8.nodeAfterCursor && !options8.nodeBeforeCursor) {
    return [prettier_cursor, doc2];
  }
  if (options8.nodeBeforeCursor && !options8.nodeAfterCursor) {
    return [doc2, prettier_cursor];
  }
  return doc2;
...
function callPluginPrintFunction(path13, options8, printPath, args, embeds) {
  ...
  switch (node) {
    ...
    case options8.nodeBeforeCursor:
      doc2 = inheritLabel(doc2, (doc3) => [doc3, prettier_cursor]);
      break;
    case options8.nodeAfterCursor:
      doc2 = inheritLabel(doc2, (doc3) => [prettier_cursor, doc3]);
      break;
  }
...
async function printDocToString2(doc2, options8) {
  return prettier_printDocToString(
    doc2,
...
var { parsers, printers } = createParsersAndPrinters([
  {
    importPlugin: () => __webpack_require__.e(/* import() */ 545).then(__webpack_require__.bind(__webpack_require__, 7545)),
    parsers: ["acorn", "espree"]
  },
  {
    importPlugin: () => __webpack_require__.e(/* import() */ 136).then(__webpack_require__.bind(__webpack_require__, 9136)),
    parsers: [
      "__ng_action",
      "__ng_binding",
      "__ng_interpolation",
  ...
  {
    importPlugin: () => __webpack_require__.e(/* import() */ 918).then(__webpack_require__.bind(__webpack_require__, 22918)),
    parsers: [
      "babel",
      "babel-flow",
      "babel-ts",
  ...
  {
    importPlugin: () => __webpack_require__.e(/* import() */ 686).then(__webpack_require__.bind(__webpack_require__, 26686)),
    printers: ["estree", "estree-json"]
  },
  {
    importPlugin: () => __webpack_require__.e(/* import() */ 866).then(__webpack_require__.bind(__webpack_require__, 69866)),
    parsers: ["flow"]
  },
  {
    importPlugin: () => __webpack_require__.e(/* import() */ 331).then(__webpack_require__.bind(__webpack_require__, 16331)),
    parsers: ["glimmer"],
    printers: ["glimmer"]
  },
  {
    importPlugin: () => __webpack_require__.e(/* import() */ 457).then(__webpack_require__.bind(__webpack_require__, 46457)),
    parsers: ["graphql"],
    printers: ["graphql"]
  },
  {
    importPlugin: () => __webpack_require__.e(/* import() */ 965).then(__webpack_require__.bind(__webpack_require__, 19965)),
    parsers: ["html", "angular", "vue", "lwc"],
    printers: ["html"]
  },
  {
    importPlugin: () => __webpack_require__.e(/* import() */ 273).then(__webpack_require__.bind(__webpack_require__, 40273)),
    parsers: ["markdown", "mdx", "remark"],
    printers: ["mdast"]
  },
  {
    importPlugin: () => __webpack_require__.e(/* import() */ 427).then(__webpack_require__.bind(__webpack_require__, 21427)),
    parsers: ["meriyah"]
  },
  {
    importPlugin: () => __webpack_require__.e(/* import() */ 801).then(__webpack_require__.bind(__webpack_require__, 30801)),
    parsers: ["css", "less", "scss"],
    printers: ["postcss"]
  },
  {
    importPlugin: () => __webpack_require__.e(/* import() */ 963).then(__webpack_require__.bind(__webpack_require__, 95963)),
    parsers: ["typescript"]
  },
  {
    importPlugin: () => __webpack_require__.e(/* import() */ 999).then(__webpack_require__.bind(__webpack_require__, 57999)),
    parsers: ["yaml"],
    printers: ["yaml"]
  }
]);
...
async function importPlugin(name, cwd) {
  if (external_path_.isAbsolute(name)) {
    return __webpack_require__(69512)((0,external_url_.pathToFileURL)(name).href);
  }
  try {
    return await __webpack_require__(69512)((0,external_url_.pathToFileURL)(external_path_.resolve(name)).href);
  } catch {
